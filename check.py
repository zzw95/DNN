...
def gradient_check(X, Y, layer_dims, layer_types, parameters, epsilon = 1e-5, num_params =8):
    """
    ...
    for l in range(1,L):
        ...
        for i in check_params:
            ...
            J = nn_model.comput_cost(Yhat, Y, parameters, layer_types[-1])
            grads = nn_model.backword_propagation(X, Y, parameters, caches, layer_dims, layer_types)
            dWl = grads['dW'+str(l)]
            print(np.sum(dWl>0))
            ...
            Yhat,caches = nn_model.forword_propagation(X, parameters, layer_dims, layer_types)
            J_plus = nn_model.comput_cost(Yhat, Y, parameters, layer_types[-1])
            print('J_plus = '+str(J_plus))

            parameters['W'+str(l)][0][i] = Wl_minus
            print('W%d_minus[%d][0] = %f' % (l, i, parameters['W'+str(l)][0,i]))
            Yhat,caches = nn_model.forword_propagation(X, parameters, layer_dims, layer_types)
            J_minus = nn_model.comput_cost(Yhat, Y, parameters, layer_types[-1])
...
def check_sigmoid():
    ...
    gradient_check(X, Y, layer_dims, layer_types, parameters)
...
def check_softmax():
    ...
    parameters = nn_model.init_params(layer_dims)
    gradient_check(X, Y_onehot, layer_dims, layer_types, parameters,epsilon = 1e-7, num_params =2)
